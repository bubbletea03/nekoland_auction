...
varStringList = {101,102,103,104,105}
-- 아이템정보 저장 스트링변수번호
	...
	for i, v in ipairs(Server.players) do
		if v.id == target_playerID then
			...
			if unit.gameMoney < item[4] then
				unit.SendCenterLabel("<Color=Red>[System]</color> 금액이 <Color=Yellow>" .. item[4]-unit.gameMoney .. "Gold</color> 만큼 모자랍니다.")
				s_market:target(v.id, v.name)
				return
			end
