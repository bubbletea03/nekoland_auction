...
c_market_ruby.title = {}
c_market_ruby.me = 0
c_market_ruby.page = 1
c_market_ruby.choose = 0

function c_market_ruby:players(ids,names,title,me)
	if ids then
		c_market_ruby.ids = Utility.JSONParse(ids)
		c_market_ruby.names = Utility.JSONParse(names)
		c_market_ruby.title = Utility.JSONParse(title)
		c_market_ruby.me = me
	end
	...
	local t00 = Text("유저 루비장터목록", Rect(0,5,p00.width,30))
	local t001 = Text("유저 루비장터목록", Rect(0,4,p00.width,30))
	local t002 = Text("유저 루비장터목록", Rect(-1,5,p00.width,30))
	local t003 = Text("유저 루비장터목록", Rect(0,6,p00.width,30))
	local t004 = Text("유저 루비장터목록", Rect(1,5,p00.width,30))
	...
	b01.onClick.Add(function()
		if self.page >= math.ceil(#self.ids/5) then
			Client.FireEvent("s_market_ruby:text", 1)
			return
		end
		mask.Destroy()
		self.page = self.page + 1
		c_market_ruby:players()
	end)
	p00.AddChild(b01)
	...
	b02.onClick.Add(function()
		if self.page <= 1 then
			Client.FireEvent("s_market_ruby:text", 2)
			return
		end
		mask.Destroy()
		self.page = self.page - 1
	...
	b03.onClick.Add(function()
		Client.FireEvent("s_market_ruby:text", 5)
		mask.Destroy()
		self.page = 1
		Client.FireEvent("s_market_ruby:players")
	end)
	p00.AddChild(b03)
	...
	for i=1+(self.page-1)*10, self.page*10 do
		...
		list.button[i].onClick.Add(function()
			if self.ids[i] == self.me then
				Client.FireEvent("s_market_ruby:text", 6)
				return
			end
			mask.Destroy()
			Client.FireEvent("s_market_ruby:target", self.ids[i], self.names[i])
		end)
		list.panel[i-(self.page-1)*10].AddChild(list.button[i])
	end
end
Client.GetTopic("c_market_ruby:players").Add(function(a,b,c,d) c_market_ruby:players(a,b,c,d) end)


function c_market_ruby:target_table(a,b,c,d,e,player_id,player_name)
	...
	local t00 = Text("<Color=Yellow>" .. player_name .. "</color> 님의 루비장터", Rect(5,5,450,30))
	t00.textAlign = 4
	t00.textSize = 15
	p00.AddChild(t00)
	...
	b01.onClick.Add(function()
		mask.Destroy()
		Client.FireEvent("s_market_ruby:GM", player_id, player_name)
	end)
	...
	for i, v in ipairs(item) do
		...
		if v ~= 0 then
			...
			list.button[i].onClick.Add(function()
				Client.ShowYesNoAlert("정말 구매하시겠습니까?\n\n" .. Client.GetItem(item[i][1]).name .. " +" .. item[i][3] .. "\n" .. item[i][2] .. "개\n" .. item[i][4] .. "루비", function(a)
					if a == 1 then
						Client.FireEvent("s_market_ruby:buyItem", i, player_id, Utility.JSONSerialize(item[i]))
						mask.Destroy()
					end
				end)
			end)
...
Client.GetTopic("c_market_ruby:target_table").Add(function(a,b,c,d,e,f,g) c_market_ruby:target_table(a,b,c,d,e,f,g) end)


function c_market_ruby:list()
	local mask = Panel(Rect(0,0,Client.width,Client.height))
	mask.showOnTop = true
	mask.SetOpacity(1)
	...
	button_text[1] = Text("루비장터", Rect(0,0,170,30))
	...
	button[2].onClick.Add(function()
		Client.FireEvent("s_market_ruby:sellItemList")
		mask.Destroy()
	end)

	button_text[2] = Text("루비장터 관리", Rect(0,0,170,30))
...
function c_market_ruby:management(market_title,a,b,c,d,e)
	...
	b02.onClick.Add(function()
		for i, v in ipairs(item) do
			if item[i] ~= 0 then
				Client.FireEvent("s_market_ruby:text", 3)
				return
			end
		end
		Client.FireEvent("s_market_ruby:text", 4)
	end)
	...
	for i, v in ipairs(item) do
		...
		if v ~= 0 then
			...
			list.button[i].pivotX = 1
			list.button[i].onClick.Add(function()
				Client.FireEvent("s_market_ruby:collectItem", i, Utility.JSONSerialize(item[i]))
			end)
			list.buttonimg[i] = Image("Pictures/카테고리버튼.png", Rect(0,0,50,30))
			...
			list.button[i].onClick.Add(function()
				self.choose = i
				Client.FireEvent("s_market_ruby:registerItem")
			end)
			list.buttonimg[i] = Image("Pictures/카테고리버튼.png", Rect(0,0,50,30))
...
Client.GetTopic("c_market_ruby:management").Add(function(a,b,c,d,e,f) c_market_ruby:management(a,b,c,d,e,f) end)


function c_market_ruby:reTitle()
	local mask = Panel(Rect(0,0,Client.width,Client.height))
	mask.showOnTop = true
	mask.SetOpacity(0)
	...
	b00.onClick.Add(function()
		...
		self.mask1.Destroy()
		self.mask1 = nil
		Client.FireEvent("s_market_ruby:reTitle", i00.text)
	end)
...
function c_market_ruby:setPrice(Titem_count)
	local mask = Panel(Rect(0,0,Client.width,Client.height))
	mask.showOnTop = true
	mask.SetOpacity(0)
	...
	b00.onClick.Add(function()
		...
		self.mask1.Destroy()
		self.mask1 = nil
		Client.FireEvent("s_market_ruby:registerItem2", math.floor(tonumber(i00.text)), self.choose, math.floor(tonumber(i01.text)))
	end)
	i00panel.AddChild(b00)
...
Client.GetTopic("c_market_ruby:setPrice").Add(function(a) c_market_ruby:setPrice(a) end)
